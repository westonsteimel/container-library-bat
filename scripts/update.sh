...
version=$(curl --silent "https://api.github.com/repos/sharkdp/bat/releases/latest" | jq -er .tag_name)
revision=$(curl --silent "https://api.github.com/repos/sharkdp/bat/commits/${version}" | jq -er .sha)
version=${version#"v"}
